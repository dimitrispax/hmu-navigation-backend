...
export class roomDAO implements IGenericDao<Room> {
    ...
    async getAll(): Promise<Room[]> {
        let response;
        try {
            response = await pool.query(`SELECT 'Feature' as type,r.id as id, ST_AsGeoJSON(ST_Transform(ST_SetSRID(r.geom, 2100), 4326))::json as geometry, json_build_object('feat_type', 'line','feat_area', ST_Area(r.geom)) as properties
